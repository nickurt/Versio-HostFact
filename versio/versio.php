...
class Versio implements IRegistrar
{
	...
	{
	$this->setApi_login();
	$this->setApi_testmodus($this->Testmode);
	 ...
	 * 
	 * @param 	string 	$domain		The name of the domain you want to extend.
	 * @param 	int		$nyears		The number of year the domain should be extended.
		...
		foreach($response['DomainsList'] as $domainlist)
		{
			$nameservers = array();
				...
				$whois = new Whois();
					...
					$whois->ownerSex = 'm';
					$whois->ownerInitials = $response['contactInfo']['initials'];
	...
	/**
   	...
   	function updateNameServers($domain, $nameservers = array()) {
		...
		if(($nameservers['ns1'] == 'nszero1.axc.nl' || $nameservers['ns2'] == 'nszero2.axc.nl') || ($nameservers['ns1'] == 'nszero1.axc.eu' || $nameservers['ns2'] == 'nszero2.axc.eu'))
		{
			$data = array(
			'dns_management' => true,
			'ns' => $ns
